...
{
private:
    //hay que hacer un miembro para el estado actual y su color, para dibujarlo diferente
    string titulo;
    int ancho;
    int alto;
    int colorEstados;
    int colorEstadoActual;
    ...
    void dibujarEstadoActual(int x, int y, string texto) {
        setcolor(colorEstadoActual);
        setfillstyle(1, colorEstadoActual);
        fillellipse(x, y, radioEstados, radioEstados);
        moveto(x, y);
        setcolor(colorTextoEstados);
        setbkcolor(colorEstadoActual);
        settextstyle(2, 0, 5);
        settextjustify(1, 1);
        outtext(texto.c_str());
    }
...
public:
    GraficadorTuring(int pAncho, int pAlto, int pCantidadEstados) {
        ...
        alto = pAlto;
        colorEstados = 1;
        colorEstadoActual= COLOR(255,0,0);
    ...
    void dibujarEstados(string estadoActual) {
        for (int i = 0; i < cantidadEstados; i++) {
            for (int j = 0; j < cantidadEstados; j++) {
                if (transiciones[i][j] != "") {
        ...
        for (int i = 0; i < cantidadEstados; i++) {
            if (nombres[i] == estadoActual){
                dibujarEstadoActual(calcularXEstado(i, radioCirculoCentral),
                    calcularYEstado(i, radioCirculoCentral),
                    estadoActual);
            }
            else{
    ...
    void asignarTransicion(int indiceOrigen, int indiceDestino, string texto) throw(runtime_error) {
        ...
        //cout << "nombre " << texto << "asignado" <<endl;
    }


    void dibujarTira(string tira)
    {
        //dibuja la tira en la pantalla
        setbkcolor(2);
        settextstyle(3, 0, 4);
        settextjustify(0, 0);
        moveto(30, alto - 10);
