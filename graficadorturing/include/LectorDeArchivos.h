#ifndef LECTORDEARCHIVOS_H
#define LECTORDEARCHIVOS_H
...
private:
    ...
    void cargarSimbolosEntrada(string pLinea) {
        ...
        for (unsigned int i=0; i < laLinea.length(); i++) {
            ...
            if (caracterLeido != ',') {
                maquinaTuring.appendSimboloEntrada(string(1, caracterLeido));
            }
        }
    }
    ...
    void cargarSimbolosMaquina(string pLinea) {
        ...
        for (unsigned int i=0; i < laLinea.length(); i++) {
            caracterLeido = laLinea.at(i);
            if (caracterLeido != ',') {
                maquinaTuring.appendSimboloMaquina(string(1, caracterLeido));
            }
        }
    }
...
public:
    ...
    void leerArchivo(string pFileName) throw (runtime_error){
        ...
        if (!file.is_open()) throw runtime_error ("NO SE PUDO ABRIR EL ARCHIVO " + pFileName + ".");
        string linea;
